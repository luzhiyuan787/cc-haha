...
async function runInputActionGates(
  ...
  if (!frontmost) {
    ...
    // cannot tell what we are about to type into — and PixelCompare noticing
    // afterwards is not the same as not doing it.
    //
    // Ported from main's `fix(security): harden local runtime boundaries`,
    // which patched this gate while it still lived in toolCalls.ts. The macOS
    // rewrite moved the legacy surface here, so the fix has to move with it or
...
export async function handleToolCall(
  ...
  const lock = await overrides.checkCuLock?.();
  if (lock) {
