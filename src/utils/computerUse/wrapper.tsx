 ...
 * caches the dispatcher, and keeps each call's ToolUseContext in async-local
 * storage. Queued calls cannot replace the active call's cancellation, UI
 * callbacks, or AppState accessors.
 *
...
type Binding = {
  ...
  dispatch: (name: string, args: unknown, signal?: AbortSignal) => Promise<CuCallToolResult>;
};
 ...
 *
 * Each dispatch owns its ToolUseContext, including while waiting in the binder
 * queue. The cached binder's callbacks resolve that async-local context when
 * they run; another queued call cannot change it.
 */
...
function tuc(): ToolUseContext {
  const context = toolUseContexts.getStore();
  if (!context) {
    throw new Error('Computer Use dispatch has no active tool context');
  }
  return context;
}

/** A host hotkey callback must not depend on a dispatch async-local store. */
export function createComputerUseEscapeHandler(controller: AbortController): () => void {
  return () => {
...
export function buildSessionContext(): ComputerUseSessionContext {
  return {
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        // The current hotkey adapter returns false, so the UI advertises
        // Ctrl+C. If a host provides Escape support, capture this turn's
        // controller: a global callback may run outside async-local context.
        // Turn-end cleanup unregisters it before the next lock acquisition.
        const escRegistered = registerEscHotkey(
          createComputerUseEscapeHandler(tuc().abortController),
        );
...
export function getComputerUseMCPToolOverrides(toolName: string): ComputerUseMCPToolOverrides {
  const call: CallOverride = async (args, context: ToolUseContext) => {
    ...
    return await dispatchComputerUseCall(dispatch, toolName, args, context);
  };
  return {
...
/** Adapt one CLI call while retaining its own cancellation and result metadata. */
export async function dispatchComputerUseCall(
  dispatch: Binding['dispatch'],
  toolName: string,
  args: unknown,
  context: ToolUseContext,
) {
  return await toolUseContexts.run(context, async () => {
    const { telemetry, ...result } = await dispatch(
      toolName,
      args,
    ...
    // The CLI model receives content blocks, not MCP structuredContent. Keep
    // sequence progress visible there as well as in the SDK metadata.
    const content: CuCallToolResult['content'] = result.structuredContent
    ...
    if (result.isError) {
      const message = content
        ...
        .map(item => item.text)
        .join('\n');
      // The normal tool executor maps thrown errors to is_error results. A
      // failure must not pass through this override as a successful receipt.
      throw new Error(message || 'Computer Use failed');
    }

    let imageNumber = 0
    const data = (await Promise.all(content.map(async item => {
      if (item.type !== 'image') {
        return [{ type: 'text' as const, text: item.type === 'text' ? item.text : '' }]
      }
      ...
      const caption = await screenshotCaption(item.data, number, toolName === 'zoom')
      return caption ? [{ type: 'text' as const, text: caption }, image] : [image]
    }))).flat()
    return {
      data,
...
async function screenshotCaption(base64: string, imageNumber: number, zoom: boolean): Promise<string | undefined> {
  try {
    // Read the encoded image that is actually being delivered. This also covers
    // image-only and deferred REPL output, where native size metadata is absent.
    // metadata() reads the header; it never resizes or re-encodes the image.
    const imageProcessor = await getImageProcessor()
    const { width, height } = await imageProcessor(Buffer.from(base64, 'base64')).metadata()
    if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width < 1 || height < 1) return
    // Number images because some model protocols move them into a separate
    // message. Describe image pixels, not action units (Windows can use %).
    return `\nImage ${imageNumber}: ${width}×${height} pixels. Image pixel bounds: top-left (0,0), bottom-right (${width - 1},${height - 1}).`
      + (zoom ? ' Zoom image for inspection; action coordinates still refer to the full-screen screenshot in the tool\'s documented units.' : '')
      + '\n'
  } catch {
    // A missing processor or unreadable header must not turn an already
    // completed native action into a tool error or discard its screenshot.
    return
  }
}
