...
相比 `v0.4.4`，本版本让 OpenAI-compatible provider 默认走更明确的直连 proxy 模式，避免系统代理或 sidecar 代理环境影响模型连接；同时补齐 reasoning effort 透传、experimental beta 开关、默认会话权限设置、`/model` 快捷切换、Skill 命令历史恢复、Windows ARM64 启动和 package-smoke 识别。123
